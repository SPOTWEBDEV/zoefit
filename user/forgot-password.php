...
// 3-step flow:
//   Step 1 — Enter email → generate 6-digit PIN → send to email
//   Step 2 — Enter PIN   → verify against DB
//   Step 3 — Modal appears → enter new password → save
    ...
    if (!verifyCsrf($_POST[CSRF_TOKEN_NAME] ?? '')) { $error = 'Invalid request.'; }
        ...
        if (!$email || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if (!$u) {
                ...
                // Generate 6-digit PIN, expire in 15 minutes
                $pin     = str_pad((string)random_int(0, 999999), 6, '0', STR_PAD_LEFT);
                $expires = date('Y-m-d H:i:s', time() + 900); // 15 min

                // Store in DB — use a password_resets table or repurpose audit / session
                // We store in a dedicated password_resets table (created if missing)
                ...
                $db->prepare(
                    "UPDATE password_resets SET used=1 WHERE user_type='user' AND record_id=? AND used=0"
                )->execute([$u['id']]);
...
if (isPost() && ($_POST['form'] ?? '') === 'verify') {
    if (!verifyCsrf($_POST[CSRF_TOKEN_NAME] ?? '')) { $error = 'Invalid request.'; }
        ...
        if (!$pin || !$email) {
            ...
            // Find valid, unused, non-expired PIN for this email
            $r = $db->prepare(
            ...
            if (!$r || !password_verify($pin, $r['pin_hash'])) {
                ...
                // PIN valid — move to step 3 (password reset modal)
    ...
    if (!verifyCsrf($_POST[CSRF_TOKEN_NAME] ?? '')) { $error = 'Invalid request.'; }
        ...
        if (!$uid || !$rid) {
            ...
            unset($_SESSION['fp_user_step'], $_SESSION['fp_user_email'],
                  $_SESSION['fp_user_reset_id'], $_SESSION['fp_user_uid']);

            $success = 'Password changed successfully! You can now log in.';
            $step    = 1; // back to step 1 / show success
        }
    }
}
...
<div class="w-full max-w-md py-8">
  ...
  <div class="fp-card p-8">
    ...
    <p class="text-gray-400 text-sm mb-6">Enter the email address on your account and we'll send you a reset PIN.</p>
    ...
    <form method="POST">
      ...
      <button type="submit" class="btn-orange mb-4">📧 Send Reset PIN</button>
    </form>
    ...
    <a href="<?= APP_URL ?>/user/login.php" class="btn-orange mb-4">→ Log In Now</a>
    ...
    <p class="text-xs text-gray-500 mb-6">Check your spam folder if it doesn't arrive. PIN expires in 15 minutes.</p>
    ...
    <form method="POST" id="pin-form">
      ...
      <div class="flex gap-2 justify-center mb-6">
        ...
        <input type="text" maxlength="1" inputmode="numeric" pattern="[0-9]"
               class="pin-box" id="pin-<?= $i ?>"
               autocomplete="off">
        <?php endfor; ?>
      </div>

      <button type="submit" id="pin-submit" class="btn-orange mb-4" disabled style="opacity:.5;cursor:not-allowed">
        ✓ Verify PIN
      </button>
    </form>

    <div class="text-center">
      <a href="?resend=1" class="text-sm text-orange-400 hover:underline">← Use a different email</a>
    </div>

    <!-- ── STEP 3: Modal is shown, show waiting state here ─ -->
    <?php elseif ($step === 3): ?>
    <h1 class="text-2xl font-black mb-1">PIN Verified ✓</h1>
    <p class="text-gray-400 text-sm">Set your new password in the panel that appeared.</p>
...
pinInputs.forEach((inp, i) => {
  ...
  inp.addEventListener('keydown', e => {
    ...
    // Allow pasting full PIN into first box
...
if (pwNew) {
  pwNew.addEventListener('input', () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (!bar) continue;
      bar.style.background = i < score ? colors[score-1] : 'rgba(255,255,255,.1)';
    }
