...
if (isPost() && verifyCsrf($_POST[CSRF_TOKEN_NAME]??'') && ($_POST['action']??'')==='generate') {
  ...
  auditLog('admin',$adminId,'generate_codes',"Generated $generated codes, batch: $batch".($assignTo?" → vendor $assignTo":'' ));
...
if (isPost() && verifyCsrf($_POST[CSRF_TOKEN_NAME]??'') && ($_POST['action']??'')==='unassign_batch') {
  ...
  if (!$batch) { $err='Batch ID required.'; }
    ...
    $vRows=$db->prepare("SELECT assigned_vendor,COUNT(*) cnt FROM codes WHERE batch_id=? AND status='assigned' GROUP BY assigned_vendor");
    $vRows->execute([$batch]); foreach($vRows->fetchAll() as $vr)
      if($vr['assigned_vendor'])
        $db->prepare("UPDATE users SET vendor_code_balance=GREATEST(0,vendor_code_balance-?) WHERE id=?")->execute([$vr['cnt'],$vr['assigned_vendor']]);
    $db->prepare("UPDATE codes SET status='unassigned',assigned_vendor=NULL,assigned_at=NULL WHERE batch_id=? AND status='assigned'")->execute([$batch]);
...
if (isset($_GET['export'])) {
  ...
  $rows=$db->prepare("SELECT c.code,c.status,c.batch_id,u.full_name owner,v.full_name vendor,c.generated_at
...
$codes=$db->prepare("SELECT c.*,u.full_name owner_name,v.full_name vendor_name FROM codes c
  LEFT JOIN users u ON c.current_owner=u.id
  LEFT JOIN users v ON c.assigned_vendor=v.id
  WHERE $where ORDER BY c.generated_at DESC LIMIT $per OFFSET $offset");
$codes->execute($params); $codes=$codes->fetchAll();

$cntS=$db->prepare("SELECT COUNT(*) FROM codes c WHERE $where"); $cntS->execute($params); $total=$cntS->fetchColumn();
...
$batches=$db->query("SELECT batch_id,COUNT(*) cnt,MIN(generated_at) gen_at,
  SUM(CASE WHEN status='assigned' THEN 1 ELSE 0 END) assigned_cnt,
  MAX(COALESCE(assigned_vendor,0)) vendor_id
  FROM codes WHERE batch_id IS NOT NULL GROUP BY batch_id ORDER BY gen_at DESC LIMIT 80")->fetchAll();
...
<head>
  ...
  <style>
    *{font-family:'Poppins',sans-serif!important}
    ...
    /* ── Printable ticket styles (only active inside #print-area) ── */
    @media print {
...
<div class="main-content">
  ...
  <div class="p-4 md:p-6">
    ...
    <div class="grid grid-cols-3 md:grid-cols-6 gap-3 mb-5">
      <?php $cs=[['Unassigned','unassigned','text-gray-300'],['Assigned','assigned','text-blue-400'],['Redeemed','redeemed','text-green-400'],['In Draw','reserved','text-yellow-400'],['Used','used','text-red-400'],['Transferred','transferred','text-purple-400']]; ?>
      <?php foreach($cs as $c): ?>
    ...
    <!-- Selection toolbar — shown when codes are checked -->
    ...
    <form method="GET" class="flex flex-wrap gap-3 mb-4">
      <div class="flex gap-2 flex-1 min-w-64">
        <input type="text" name="q" class="form-control font-mono" placeholder="Search code digits or paste BATCH-…" value="<?= e($q) ?>">
    ...
    <div class="flex gap-2 mb-5 flex-wrap">
      ...
      <?php endforeach; ?>
      <a href="?export=csv&filter=<?=e($filter)?><?=$q?'&q='.urlencode($q):''?>" class="btn btn-sm btn-secondary ml-auto text-gray-400">⬇ Export CSV</a>
    </div>
    ...
    <div class="card">
      <div class="table-wrap">
        <table id="codes-table">
          <thead>
            <tr>
              <th style="width:40px"><input type="checkbox" id="chk-all" class="w-4 h-4 accent-orange-500" onchange="toggleAll(this)"></th>
          ...
          <tbody>
            ...
            <tr class="code-row">
              <td><input type="checkbox" class="code-chk w-4 h-4 accent-orange-500" value="<?= e($c['code']) ?>" onchange="updateSel()"></td>
              <td><code class="text-orange-400 font-bold tracking-widest text-sm select-all"><?= e($c['code']) ?></code></td>
              <td>
                <span class="badge <?= match($c['status']){'unassigned'=>'badge-muted','assigned'=>'badge-info','redeemed'=>'badge-success','reserved'=>'badge-warning','used'=>'badge-danger','transferred'=>'badge-muted',default=>'badge-muted'} ?>">
                  <?= $c['status'] ?>
                </span>
              </td>
              ...
              <td>
                <?php if($c['batch_id']): ?>
                <button onclick="document.querySelector('[name=q]').value='<?= e($c['batch_id']) ?>';this.closest('form')?.submit();window.location='?filter=all&q=<?= urlencode($c['batch_id']) ?>'" class="text-xs text-cyan-400 hover:underline font-mono truncate max-w-[110px] block"><?= e($c['batch_id']) ?></button>
            ...
            <?php endforeach; ?>
            <?php if(!$codes): ?>
            <tr><td colspan="7" class="text-center text-gray-500 py-10">No codes found<?= $q?' for "'.e($q).'"':'' ?></td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
  ...
  <div class="modal-box" style="max-width:520px">
    ...
    <form method="POST">
      ...
      <div class="form-group">
        ...
        <select name="assign_vendor" class="form-control" onchange="this.nextElementSibling.classList.toggle('hidden',this.value==='0')">
          <option value="0">— Keep unassigned —</option>
          <?php foreach($vendors as $v): ?>
          <option value="<?= $v['id'] ?>"><?= e($v['full_name']) ?><?= $v['vendor_business_name']?' — '.e($v['vendor_business_name']):'' ?> (<?= $v['vendor_code_balance'] ?> codes)</option>
          <?php endforeach; ?>
        </select>
        <div class="hidden mt-2 p-2 bg-purple-500/10 border border-purple-500/20 rounded-lg text-xs text-purple-300">Codes will be assigned directly to this vendor's inventory.</div>
      </div>
...
<div class="modal-overlay" id="unassign-modal">
  <div class="modal-box" style="max-width:540px">
    ...
    <p class="text-gray-400 text-sm mb-4">Returns assigned codes back to the unassigned pool. Only <code class="text-blue-400">assigned</code> codes are affected.</p>
    <form method="POST">
      ...
      <div class="form-group">
        ...
        <div class="max-h-44 overflow-y-auto space-y-1 p-1">
          ...
          <button type="button" onclick="document.getElementById('unassign-batch-input').value='<?= e($b['batch_id']) ?>'"
            ...
            <div class="flex items-center justify-between gap-3">
              ...
              <?php if($b['assigned_cnt']>0): ?><span class="badge badge-info text-xs flex-shrink-0"><?= $b['assigned_cnt'] ?> assigned</span><?php endif; ?>
            </div>
            <div class="text-xs text-gray-500 mt-0.5"><?= $b['cnt'] ?> total · <?= date('M j, Y',strtotime($b['gen_at'])) ?></div>
          </button>
          <?php endforeach; ?>
        </div>
      </div>
      ...
      <div class="flex gap-3">
        ...
        <button type="submit" class="btn btn-sm flex-1 py-3 font-bold text-white" style="background:#f59e0b" onclick="return confirm('Unassign all assigned codes in this batch?')">🔓 Unassign</button>
      </div>
    </form>
  </div>
</div>
...
// ── Print tickets ───────────────────────────────────────────
// Generates a printable slip sheet — admin cuts along dotted lines and hands to users
function printTickets() {
  if (!selectedCodes.size) { Toast.error('Select codes to print first.'); return; }
  const grid = document.getElementById('ticket-grid');
  grid.innerHTML = '';
  [...selectedCodes].forEach(code => {
    // Format: groups of 5 digits for readability e.g. 74208-19356-27401
