...
// V1 MODE: Vendor sections are hidden from nav but pages are kept on disk.
// Set ZF_SHOW_VENDOR_NAV = true in config.php to re-enable when ready.

$currentPage  = $currentPage  ?? '';
$showVendor   = defined('ZF_SHOW_VENDOR_NAV') ? ZF_SHOW_VENDOR_NAV : false;
$isVendor     = false;
$vendorStatus = null;
...
if ($showVendor && !empty($_SESSION['user_id'])) {
  ...
  // Sync from DB if session not set
  if (!$isVendor) {
    try {
      $sv = getDB()->prepare("SELECT is_vendor,vendor_status FROM users WHERE id=?");
      $sv->execute([$_SESSION['user_id']]);
      $sv = $sv->fetch();
      if ($sv) {
        $_SESSION['is_vendor']     = (bool)$sv['is_vendor'];
        $_SESSION['vendor_status'] = $sv['vendor_status'];
        $isVendor     = (bool)$sv['is_vendor'];
        $vendorStatus = $sv['vendor_status'];
      }
    } catch (\Exception $e) {
    }
  }
...
<nav class="sidebar flex flex-col" id="user-sidebar">
  ...
  <div class="flex-1 py-3 overflow-y-auto space-y-0.5">
    ...
    <?php if ($showVendor): ?>
      <!-- ── VENDOR SECTION (visible when ZF_SHOW_VENDOR_NAV = true) ── -->
      ...
      <?php else: ?>
        ...
        <a href="<?= APP_URL ?>/user/vendor-apply.php" class="nav-item text-orange-400 hover:bg-orange-500/10 <?= $currentPage === 'vendor-apply' ? 'active' : '' ?>">
          <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
          </svg>
          Become a Vendor
...
<!-- ── Beautiful Bottom Navigation (mobile only) ─────────── -->
<nav class="md:hidden fixed bottom-0 left-0 right-0 z-50"
  ...
  <div class="flex items-stretch h-16">
    ...
    <a href="<?= APP_URL ?>/user/services.php" class="bnav-item <?= $currentPage === 'services' ? 'bnav-active' : '' ?>">
      <svg class="bnav-icon" fill="<?= $currentPage === 'services' ? 'currentColor' : 'none' ?>" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
      </svg>
      <span>Services</span>
    </a>

    <a href="<?= APP_URL ?>/user/profile.php" class="bnav-item <?= in_array($currentPage, ['profile', 'notifications', 'transactions']) ? 'bnav-active' : '' ?>">
      <div class="relative">
