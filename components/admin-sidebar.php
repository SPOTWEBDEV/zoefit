...
try {
  ...
  $_s->execute();
  $_pendingVendors = (int)$_s->fetchColumn();
} catch (\Exception $e) {
}
?>
<nav class="sidebar flex flex-col" id="user-sidebar">
  ...
  <div class="flex-1 py-2 overflow-y-auto space-y-0.5">
    ...
    <a href="<?= APP_URL ?>/admin/winners.php" class="nav-item <?= $aPage === 'winners' ? 'active' : '' ?>">
      <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  <div class="border-t border-white/5 px-3 py-3">
    <div class="flex items-center gap-2 mb-2 px-2">
      ...
      <div class="min-w-0">
        <div class="text-xs font-semibold truncate"><?= e($_SESSION['admin_name'] ?? 'Admin') ?></div>
